...
import com.julun.huanque.common.bean.beans.BoxGainGift
 ...
 */
class BlindResultItemAdapter : BaseQuickAdapter<BoxGainGift, BaseViewHolder>(R.layout.item_egg_result) {

    var mWidth = ((ScreenUtils.getScreenWidth() - dp2px(30) * 2) / 3.5).toInt()

    override fun convert(helper: BaseViewHolder, item: BoxGainGift) {
        ...
        tv_name.show()
        tv_num.show()
        ImageUtils.loadImage(sdv_gift, item.prodPic, 46f, 46f)
        tv_name.text = item.prodName
        tv_num.text = "x${item.prodCnt}"
