...
public class ListViewHolder {
    ...
    public ListViewHolder setText(int viewId, CharSequence txt) {
        TextView tv = getView(viewId);
        tv.setText(txt);
        return this;
    }
    ...
    public ListViewHolder fuckOff(int viewId) {
        View view = getView(viewId);
    ...
    public ListViewHolder showMe(int viewId, boolean show) {
        return show ? showMe(viewId) : fuckOff(viewId);
    }

    public ListViewHolder showMe(int viewId) {
        View view = getView(viewId);
    ...
    public ListViewHolder setImageUrl4DraweeView (int cmpId, @NotNull String ossImgUrl) {
        SimpleDraweeView image = getView (cmpId);
        if(image != null){
            image.setImageURI (ossImgUrl);
        }
        return this;
    }


    public ListViewHolder setImageUrl4DraweeViewInPx (int cmpId, @NotNull String simpleUrl,int width,int height) {
        SimpleDraweeView image = getView (cmpId);
        if(image != null){
            ImageUtils.INSTANCE.loadImageInPx (image, StringHelper.INSTANCE.getOssImgUrl (simpleUrl),width,height);
//            image.setImageURI (ossImgUrl);
        }
        return this;
    }
