...
public class NumberAnimTextView extends TextView {
    ...
    private boolean isNeedFormat = true;//是否需要格式化数字，默认需要
    ...
    private void start() {
        if (!isEnableAnim) { // 禁止动画
            if (isNeedFormat()) {
                ...
                setText(mPrefixString + new BigDecimal(mNumEnd).intValue() + mPostfixString);
            }
            return;
        }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                if (isNeedFormat()) {
                    ...
                    setText(mPrefixString + value.intValue() + mPostfixString);
                }
            }
        });
        animator.start();
