...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    public void setHeaderViewBackgroundDrawable(Drawable drawable) {
        mHeadViewContainer.setBackgroundDrawable(drawable);
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (pointCount <= 0) {
            //接收到触摸事件，但是触摸点不存在，表示出现未知异常  直接返回false
            return false;
        }
        final int action = MotionEventCompat.getActionMasked(ev);
...
//            return handlerPushTouchEvent(ev, action);
     ...
     * 下拉刷新回调
     */
    protected interface OnPullRefreshListener {
